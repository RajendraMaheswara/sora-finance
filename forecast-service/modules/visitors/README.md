...
```
forecast-service/modules/visitors/
│
├── app/
│   ├── api/
...
│
├── ../../models/visitors/             # Model .joblib tersimpan di sini
├── logs/                            # Log files
│
...
```bash
cd forecast-service/modules/visitors
python -m venv venv
...
COPY . .
RUN mkdir -p ../../models/visitors logs
EXPOSE 5000
...
docker run -d \
  ...
  -v $(pwd)/../../models/visitors:/app/models/visitors \
  -v $(pwd)/logs:/app/logs \
  --name sora-forecast \
  sora-forecast
```
